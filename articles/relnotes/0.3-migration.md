...
Isso permite um novo conjunto de recursos do IntelliSense, juntamente com os comentários ao vivo sobre a digitação de sublinhados ondulados de erros e avisos.
...
Essa atualização melhora muito as mensagens de diagnóstico em geral, com navegação fácil e intervalos precisos para diagnóstico e detalhes adicionais nas informações de foco exibidas. 
...
5. Depois de executar isso, talvez você ainda precise resolver manualmente as alterações semânticas nos casos em que a interpretação semântica do código foi alterada. Todos esses erros serão relatados pelo IntelliSense no Visual Studio ou Visual Studio Code.
    ...
    - Depois de abrir um arquivo. QS no editor, você deverá ver a saída da extensão da linguagem Q # na janela saída.
    - Depois que o projeto for carregado com êxito (indicado na janela de saída), abra cada arquivo e manualmente para resolver todos os problemas restantes.
