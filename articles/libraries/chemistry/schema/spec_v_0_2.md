...
Se o valor de `index_convention` for `mulliken`, para cada elemento da quantidade de matriz esparsa de `two_electron_integrals`, um analisador que carrega um documento Broombridge deverá criar uma instância de um termo Hamiltonian igual ao operador de dois bits, $h _ {i, j, k, l} a ^ \ dagger_i um ^ \ dagger_j a_k a_l $, em que $i $, $j $, $k $ e $l $ devem ser inteiros de valor pelo menos 1 e onde $h _ {i, j, k, l} $ é o elemento `[i, j, k, l, h(i, j, k, l)]` da quantidade de matriz esparsa.
