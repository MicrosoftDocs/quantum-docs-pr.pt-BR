...
Autor: bradben MS. Author: v-benbra MS. Date: 9/1/2020 MS. Topic: UID de referência: Microsoft. Quantum. Glossário no-loc:
- "Q#"
- "$$v"
- "$$"
- "$$"
...
Uma representação gráfica de um estado de[](xref:microsoft.quantum.glossary#qubit) [Quantum](xref:microsoft.quantum.glossary#quantum-state) de qubit único como um ponto em uma esfera de unidade tridimensional. Para obter mais informações, consulte  [visualizando qubits e transformações usando a esfera Bloch](xref:microsoft.quantum.concepts.qubit#visualizing-qubits-and-transformations-using-the-bloch-sphere).

## <a name="callable"></a>Acessível

Uma [operação](xref:microsoft.quantum.glossary#operation) ou [função](xref:microsoft.quantum.glossary#function) no [ Q# idioma](https://github.com/microsoft/qsharp-language/tree/main/Specifications/Language#q-language).
Para obter mais informações, consulte [ Q# programas](xref:microsoft.quantum.guide.programs)
...
Uma [operação](xref:microsoft.quantum.glossary#operation) Quantum que usa um ou mais [qubits](xref:microsoft.quantum.glossary#qubit) como habilitadores para a operação de destino. Para obter mais informações, consulte [functor Application](xref:microsoft.quantum.qsharp.functorapplication#functor-application).
...
Um termo herdado para determinadas [operações](xref:microsoft.quantum.glossary#operation)de Quantum intrínsecas, com base no conceito de Gates de lógica clássica. Um [circuito Quantum](xref:microsoft.quantum.glossary#quantum-circuit-diagram) é uma rede de Gates, com base no conceito semelhante de circuitos lógicos clássicos.
...
A unidade básica de computação Quantum no Q# . Ele é basicamente equivalente a uma função em C, C++ ou Python, ou um método estático em C# ou Java. Para obter mais informações, consulte [ Q# programas](xref:microsoft.quantum.guide.programs).
...
Um método para representar graficamente a sequência de [Gates](xref:microsoft.quantum.glossary#gate) para programas Quantum simples, por exemplo 
...
Um conceito geralmente usado em algoritmos Quantum que consiste em aplicar repetidamente uma computação até que uma determinada condição seja satisfeita. Quando a condição não for satisfeita, geralmente uma correção é necessária antes de tentar novamente inserindo a próxima iteração. Para obter mais informações, consulte o [ Q# Guia do usuário](xref:microsoft.quantum.guide)
...
Um tipo personalizado que pode conter um ou mais itens nomeados ou anônimos. Para obter mais informações, consulte [declarações de tipo] Microsoft. Quantum. qsharp. typedeclarations # Type-Declarations).
