...
## <a name="version-0152101126807"></a>0.15.2101.126807 da versão

*Data de lançamento: 29 de janeiro de 2021*

- Modelos de projeto adicionados ao Q# compilador para executáveis direcionados a provedores IonQ e Honeywell
- Atualizar I Q# realce da sintaxe do kernel para incluir alterações Q# na sintaxe introduzida na versão [0.15.2101125897](#version-0152101125897)
- Bugfix para dar suporte à passagem de matrizes como argumentos de entrada para Q# programas enviados para o Quantum do Azure por meio do `%azure.execute` , consulte o problema [#401](https://github.com/microsoft/iqsharp/issues/401)
- Corrigir o erro "permissão negada" encontrado usando `az` dentro de `iqsharp-base` imagens do Docker, consulte Issue [#404](https://github.com/microsoft/iqsharp/issues/404)

## <a name="version-0152101125897"></a>0.15.2101125897 da versão

*Data de lançamento: janeiro de 26, 2021*

- Alocação qubit simplificada, fornecendo uma sintaxe mais conveniente para alocar qubits, [consulte os detalhes no Q# repositório de idioma](https://github.com/microsoft/qsharp-language/blob/main/Approved/1-implicitly-scoped-qubit-allocation.md).
- Criado QDK-Python repositório que inclui `azure-quantum` o, o cliente Python para enviar trabalhos de otimização inspirados por Quantum para o serviço Quantum do Azure, bem como `qdk` , incluindo `qdk.chemistry` , uma camada de conveniência baseada em Python para a Q# biblioteca química que inclui visualização e funcionalidade de molecular para gerar arquivos de entrada para vários pacotes de química, como NWChem, Psi4 e OpenMolcas.
- Os parênteses agora são opcionais para a operação e tipos de função e `if` instruções, e `elif` `while` `until` . Parênteses para `for` , `use` e as `borrow` instruções foram preteridas.
- Estimativas de largura aprimoradas para profundidade ideal, [Consulte detalhes](https://github.com/MicrosoftDocs/quantum-docs-pr/pull/1159).
- Aplicar a operação unitário fornecida como matriz explícita usando `ApplyUnitary` ([QuantumLibraries # 391](https://github.com/microsoft/QuantumLibraries/pull/391), contribuição externa por Dmytro Fedoriaka)
- Corrigido https://github.com/microsoft/iqsharp/issues/387 pela redução do impacto no desempenho na Q# inicialização do kernel.

## <a name="version-0142011120240"></a>0.14.2011120240 da versão

*Data de lançamento: 25 de novembro de 2020*

- Melhor desempenho do compilador devido ao carregamento mais rápido da referência.
- Adicionada uma [gramática antlr para Q# ](https://github.com/microsoft/qsharp-language/tree/main/Specifications/Language/5_Grammar) a Q# especificação da linguagem.
- Atualizado o [ `Microsoft.Quantum.Preparation` namespace](xref:Microsoft.Quantum.Preparation) para ser mais consistente com o guia de estilo e os princípios de design da API e para dar suporte a Estados mistos do purified com dados adicionais (consulte [proposta](https://github.com/microsoft/QuantumLibraries/issues/344), [notas de revisão](https://github.com/microsoft/QuantumLibraries/blob/main/Design/meetings/2020/api-design-2020-11-05.md) e PRS [#212](https://github.com/microsoft/QuantumLibraries/pull/212), [#322](https://github.com/microsoft/QuantumLibraries/pull/322), [#375](https://github.com/microsoft/QuantumLibraries/pull/375) [#376](https://github.com/microsoft/QuantumLibraries/pull/376)).
- Os parênteses em volta de expressões de chamada repetidas agora são opcionais: `(Foo(x))(y)` podem ser escritos como `Foo(x)(y)` .
- Os usuários do Visual Studio ou do Visual Studio Code Extensions que instalaram o .NET 5 ou o Visual Studio 16,8 podem ser solicitados a instalar o .NET Core 3,1 para continuar a trabalhar com as extensões.
...
## <a name="version-01320111004"></a>0.13.20111004 da versão

*Data de lançamento: 10 de novembro de 2020*

Esta versão desabilita os recursos do IntelliSense para Q# arquivos no Visual Studio e Visual Studio Code quando um arquivo de projeto não está presente. Isso resolve um problema em que os recursos do IntelliSense podem parar de funcionar depois de adicionar um novo Q# arquivo a um projeto (consulte [qsharp-Compiler # 720](https://github.com/microsoft/qsharp-compiler/issues/720)).

## <a name="version-01320102604"></a>0.13.20102604 da versão

*Data de lançamento: outubro de 27, 2020*

Esta versão contém o seguinte:

- A estimativa de recursos agora emite estimativas de profundidade e largura simultâneas, além da contagem de qubit. Confira [aqui](xref:microsoft.quantum.machines.resources-estimator#metrics-reported) para obter detalhes.
...
- A especificação de anúncio e rascunho da [representação intermediária Quantum (QIR)](https://github.com/microsoft/qsharp-language/tree/main/Specifications/QIR#quantum-intermediate-representation-qir) foi criada como um formato comum entre diferentes front-and-ends. Veja também nossa [postagem no blog](https://devblogs.microsoft.com/qsharp/introducing-quantum-intermediate-representation-qir) em QIR.
- Lançamento do nosso novo [ Q# repositório de linguagem](https://github.com/microsoft/qsharp-language) que contém também a [ Q# documentação](https://github.com/microsoft/qsharp-language/tree/main/Specifications/Language#q-language)completa.
- Melhorias de desempenho do QuantumSimulator para programas que envolvem um grande número de qubits: melhor aplicativo de decisões do Gate Fusion; maior paralelização no sistema Linux; agendamento inteligente adicionado de execução de portão; correções de bugs.
- Agora há suporte para os recursos do IntelliSense para Q# arquivos no Visual Studio e Visual Studio Code mesmo sem um arquivo de projeto.
- Vários Q# aprimoramentos de interoperabilidade/Python e correções de bugs, incluindo melhor suporte para tipos de dados numpy.
- Melhorias no namespace Microsoft. Quantum. arrays (consulte [Microsoft/QuantumLibraries # 313](https://github.com/microsoft/QuantumLibraries/issues/313)).
- Adicionada uma nova [amostra repetir-até-êxito](https://github.com/microsoft/Quantum/tree/main/samples/algorithms/repeat-until-success) que usa apenas duas qubits.

Desde a última versão, a ramificação padrão em cada um de nossos repositórios de código-fonte aberto foi renomeada para `main` .
...
- Novo suporte para modificadores de acesso no Q# , para obter mais informações, consulte [modificadores de acesso](xref:microsoft.quantum.qsharp.accessmodifiers)
...
- Novo atributo de teste para Q# testes de unidade, consulte a documentação da API atualizada [aqui](xref:Microsoft.Quantum.Diagnostics.Test) e os testes atualizados & guia de depuração [aqui](xref:microsoft.quantum.guide.testingdebugging)
...
- Novo suporte para [instruções de conjugação](xref:microsoft.quantum.qsharp.conjugations#conjugations) em Q#
...
- Nova indexação para matrizes de fatias, [consulte a referência de linguagem](xref:microsoft.quantum.qsharp.contextualexpressions#contextual-and-omitted-expressions) para obter mais informações.
...
* Adicionar itens nomeados para [tipos definidos pelo usuário] Microsoft. Quantum. qsharp. typedeclarations # Type-Declarations).  
* Agora, os construtores de tipos definidos pelo usuário podem ser usados como funções.
* Adição de suporte para [copy-and-update](xref:microsoft.quantum.qsharp.copyandupdateexpressions#copy-and-update-expressions) e [apply-and-reassign](xref:microsoft.quantum.qsharp.variabledeclarationsandreassignments#evaluate-and-reassign-statements) em tipos definidos pelo usuário.
* Agora, o bloco de correção para loops de [repeat-until-success](xref:microsoft.quantum.qsharp.conditionalloops#repeat-statement) é opcional.
...
* Adicionar um novo operador ternário para [Copy-and-Update](xref:microsoft.quantum.qsharp.copyandupdateexpressions#copy-and-update-expressions), `w/` `<-` pode ser usado para expressar a criação da matriz como uma modificação de uma matriz existente.
* Adicione a [instrução de aplicação-e-reatribuição](xref:microsoft.quantum.qsharp.variabledeclarationsandreassignments#evaluate-and-reassign-statements)comum, por exemplo, `+=` , `w/=` .
* Adicionar uma maneira de especificar um nome curto para namespaces em [diretivas abertas](xref:microsoft.quantum.qsharp.namespaces#open-directives).
...
- Foi adicionada a capacidade de retornar de dentro de uma [instrução using-and com empréstimo](xref:microsoft.quantum.qsharp.quantummemorymanagement#quantum-memory-management).
...
- adiciona suporte para um novo tipo primitivo, BigInt, que representa um inteiro com sinal de tamanho arbitrário.  Saiba mais sobre [bigint](xref:microsoft.quantum.qsharp.valueliterals#bigint-literals).
