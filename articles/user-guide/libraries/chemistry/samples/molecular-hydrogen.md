...
Agora passamos o `qSharpData` que representa o Hamiltonian para a `TrotterStepOracle` função na [simulação do Hamiltonian Dynamics](xref:microsoft.quantum.libraries.standard.algorithms). `TrotterStepOracle`Retorna uma operação Quantum que aproxima a evolução em tempo real do Hamiltonian.
...
Este código de Q # agora pode ser invocado a partir do programa de driver. A seguir, criamos um simulador de estado completo e executamos `GetEnergyByTrotterization` para obter a energia de estado terrestre.
...
Observe que dois parâmetros são retornados. `energyEst`é a estimativa da energia do estado do solo e deve estar `-1.137` em média. `phaseEst`é a fase bruta retornada pelo algoritmo de estimativa de fase e é útil para diagnosticar quando o alias ocorre devido a um `trotterStep` que é muito grande.
