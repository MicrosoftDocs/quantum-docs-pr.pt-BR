...
Depois de clonado `Microsoft/Quantum` , execute `cd` na `utilities/` pasta e importe o módulo do PowerShell para trabalhar com Docker e NWChem:
...
Agora você deve ter o `Convert-NWChemToBroombridge` comando disponível:
...
Agora você deve ter o `Get-GateCount` comando disponível:
...
> [!NOTE]
> Como esse é um repositório muito grande, podemos fazer um clone superficial para economizar alguma largura de banda e espaço em disco, usando o `--depth 1` argumento.
...
Para executar o NWChem e produzir um documento Broombridge para o `h4_sto6g_0.000.nw` baralho de entrada, execute `Convert-NWChemToBroombridge` :

> [!NOTE]
> Na primeira vez que você executar esse comando, o Docker baixará a `nwchemorg/nwchem-qc` imagem para você.
