---
title: 'Operações e funções no :::no-loc(Q#):::'
...
---
# <a name="operations-and-functions-in-no-locq"></a><span data-ttu-id="37356-103">Operações e funções no :::no-loc(Q#):::</span><span class="sxs-lookup"><span data-stu-id="37356-103">Operations and Functions in :::no-loc(Q#):::</span></span>
...
<span data-ttu-id="37356-105">As operações são o núcleo de :::no-loc(Q#)::: .</span><span class="sxs-lookup"><span data-stu-id="37356-105">Operations are the core of :::no-loc(Q#):::.</span></span>
<span data-ttu-id="37356-106">Depois de declarados, eles podem ser chamados de aplicativos .NET clássicos, por exemplo, usando um simulador ou outras operações no :::no-loc(Q#)::: .</span><span class="sxs-lookup"><span data-stu-id="37356-106">Once declared, they can either be called from classical .NET applications, for example, using a simulator or by other operations within :::no-loc(Q#):::.</span></span>
<span data-ttu-id="37356-107">Cada operação definida no :::no-loc(Q#)::: pode chamar qualquer número de outras operações, incluindo as operações intrínsecas internas definidas pelo idioma.</span><span class="sxs-lookup"><span data-stu-id="37356-107">Each operation defined in :::no-loc(Q#)::: can call any number of other operations, including the built-in intrinsic operations defined by the language.</span></span> <span data-ttu-id="37356-108">A maneira específica de :::no-loc(Q#)::: definir essas operações intrínsecas depende da máquina de destino.</span><span class="sxs-lookup"><span data-stu-id="37356-108">The particular way in which :::no-loc(Q#)::: defines these intrinsic operations depends on the target machine.</span></span>
...
<span data-ttu-id="37356-110">Cada :::no-loc(Q#)::: arquivo de origem pode definir qualquer número de operações.</span><span class="sxs-lookup"><span data-stu-id="37356-110">Each :::no-loc(Q#)::: source file can define any number of operations.</span></span>
...
> [!NOTE]
> <span data-ttu-id="37356-124">Cada operação no :::no-loc(Q#)::: usa exatamente uma entrada e retorna exatamente uma saída.</span><span class="sxs-lookup"><span data-stu-id="37356-124">Each operation in :::no-loc(Q#)::: takes exactly one input and returns exactly one output.</span></span>
...
> <span data-ttu-id="37356-126">Nesse sentido, :::no-loc(Q#)::: é uma linguagem de "tupla na tupla".</span><span class="sxs-lookup"><span data-stu-id="37356-126">In this regard, :::no-loc(Q#)::: is a "tuple-in tuple-out" language.</span></span>
...
<span data-ttu-id="37356-140">A seção a seguir descreve como chamar essas várias especializações em seu :::no-loc(Q#)::: código e como definir operações para dar suporte a elas.</span><span class="sxs-lookup"><span data-stu-id="37356-140">The following section describes how to call these various specializations in your :::no-loc(Q#)::: code, and how to define operations to support them.</span></span>
...
<span data-ttu-id="37356-142">Um *functor* no :::no-loc(Q#)::: é uma fábrica que define uma nova operação de outra operação.</span><span class="sxs-lookup"><span data-stu-id="37356-142">A *functor* in :::no-loc(Q#)::: is a factory that defines a new operation from another operation.</span></span>
<span data-ttu-id="37356-143">Os dois transmissão functors padrão no :::no-loc(Q#)::: são `Adjoint` e `Controlled` .</span><span class="sxs-lookup"><span data-stu-id="37356-143">The two standard functors in :::no-loc(Q#)::: are `Adjoint` and `Controlled`.</span></span>
...
<span data-ttu-id="37356-145">Portanto, o transmissão functors pode executar funções mais complexas do que as funções tradicionais de nível superior.</span><span class="sxs-lookup"><span data-stu-id="37356-145">Thus, functors can perform more complex functions than traditional higher-level functions.</span></span> <span data-ttu-id="37356-146">Transmissão functors não tem uma representação no sistema de :::no-loc(Q#)::: tipos.</span><span class="sxs-lookup"><span data-stu-id="37356-146">Functors do not have a representation in the :::no-loc(Q#)::: type system.</span></span> <span data-ttu-id="37356-147">Portanto, no momento, não é possível associá-los a uma variável ou passá-los como argumentos.</span><span class="sxs-lookup"><span data-stu-id="37356-147">It is thus currently not possible to bind them to a variable or pass them as arguments.</span></span> 
...
> [!NOTE]
> <span data-ttu-id="37356-159">No :::no-loc(Q#)::: , as versões controladas sempre assumem uma matriz de qubits de controle e o controle é sempre baseado em todos os qubits de controle que estão no `PauliZ` estado computacional () `One` , $ \ket {1} $.</span><span class="sxs-lookup"><span data-stu-id="37356-159">In :::no-loc(Q#):::, controlled versions always take an array of control qubits, and the controlling is always based on all of the control qubits being in the computational (`PauliZ`) `One` state, $\ket{1}$.</span></span>
...
<span data-ttu-id="37356-201">:::no-loc(Q#)::: as operações podem conter as seguintes declarações de especialização explícitas:</span><span class="sxs-lookup"><span data-stu-id="37356-201">:::no-loc(Q#)::: operations can contain the following explicit specialization declarations:</span></span>
...
<span data-ttu-id="37356-257">:::no-loc(Q#)::: é *Covariance* com relação a tipos de retorno que podem ser chamados: um callable que retorna um tipo `'A` é compatível com um callable com o mesmo tipo de entrada e um tipo de resultado compatível com `'A` .</span><span class="sxs-lookup"><span data-stu-id="37356-257">:::no-loc(Q#)::: is *covariant* with respect to callable return types: a callable that returns a type `'A` is compatible with a callable with the same input type and a result type that is compatible with `'A` .</span></span>

<span data-ttu-id="37356-258">:::no-loc(Q#)::: é *contravariant* em relação aos tipos de entrada: um callable que usa um tipo `'A` como entrada é compatível com um callable com o mesmo tipo de resultado e um tipo de entrada compatível com `'A` .</span><span class="sxs-lookup"><span data-stu-id="37356-258">:::no-loc(Q#)::: is *contravariant* with respect to input types: a callable that takes a type `'A` as input is compatible with a callable with the same result type and an input type that is compatible with `'A`.</span></span>
...
> [!IMPORTANT]
> <span data-ttu-id="37356-264">:::no-loc(Q#)::: 0,3 introduziu uma diferença significativa no comportamento de tipos definidos pelo usuário.</span><span class="sxs-lookup"><span data-stu-id="37356-264">:::no-loc(Q#)::: 0.3 introduced a significant difference in the behavior of user-defined types.</span></span>
...
<span data-ttu-id="37356-270">Como consequência, as :::no-loc(Q#)::: funções são *puras* , pois elas sempre mapeiam os mesmos valores de entrada para os mesmos valores de saída.</span><span class="sxs-lookup"><span data-stu-id="37356-270">As a consequence, :::no-loc(Q#)::: functions are *pure* , in that they always map the same input values to the same output values.</span></span>
<span data-ttu-id="37356-271">Esse comportamento permite que o :::no-loc(Q#)::: compilador reordene com segurança como e quando chamar funções ao gerar especializações de operação.</span><span class="sxs-lookup"><span data-stu-id="37356-271">This behavior allows the :::no-loc(Q#)::: compiler to safely reorder how and when to call functions when generating operation specializations.</span></span>

<span data-ttu-id="37356-272">Cada :::no-loc(Q#)::: arquivo de origem pode definir qualquer número de funções.</span><span class="sxs-lookup"><span data-stu-id="37356-272">Each :::no-loc(Q#)::: source file can define any number of functions.</span></span>
...
<span data-ttu-id="37356-280">Para enfatizar a diferença entre funções e operações, considere o problema de amostragem clássica de um número aleatório de dentro de uma :::no-loc(Q#)::: operação:</span><span class="sxs-lookup"><span data-stu-id="37356-280">To underscore the difference between functions and operations, consider the problem of classically sampling a random number from within a :::no-loc(Q#)::: operation:</span></span>
...
<span data-ttu-id="37356-289">Para implementar isso no :::no-loc(Q#)::: , aproveite o fato de que as funções são a primeira classe.</span><span class="sxs-lookup"><span data-stu-id="37356-289">To implement this in :::no-loc(Q#):::, take advantage of the fact that functions are first class.</span></span>
...
<span data-ttu-id="37356-298">Efetivamente, você deseja que o compilador trate `Map` como algum tipo de função matemática de :::no-loc(Q#)::: *tipos* para :::no-loc(Q#)::: funções.</span><span class="sxs-lookup"><span data-stu-id="37356-298">Effectively, you want the compiler to treat `Map` as some kind of mathematical function from :::no-loc(Q#)::: *types* to :::no-loc(Q#)::: functions.</span></span>

<span data-ttu-id="37356-299">:::no-loc(Q#)::: formalizes essa noção permitindo que funções e operações tenham *parâmetros de tipo* , bem como seus parâmetros de tupla comuns.</span><span class="sxs-lookup"><span data-stu-id="37356-299">:::no-loc(Q#)::: formalizes this notion by allowing functions and operations to have *type parameters* , as well as their ordinary tuple parameters.</span></span>
...
<span data-ttu-id="37356-305">No :::no-loc(Q#)::: , isso é escrito pela adição de colchetes angulares (isto é `<>` , não brakets $ \braket {} $!) após o nome de uma função ou operação em sua declaração e listando cada parâmetro de tipo.</span><span class="sxs-lookup"><span data-stu-id="37356-305">In :::no-loc(Q#):::, this is written by adding angle brackets (that's `<>`, not brakets $\braket{}$!) after the name of a function or operation in its declaration, and by listing each type parameter.</span></span>
...
<span data-ttu-id="37356-318">As :::no-loc(Q#)::: bibliotecas padrão fornecem uma variedade de operações e funções parametrizadas por tipo para facilitar o expressar o fluxo de controle de ordem mais alta.</span><span class="sxs-lookup"><span data-stu-id="37356-318">The :::no-loc(Q#)::: standard libraries provide a range of such type-parameterized operations and functions to make higher-order control flow easier to express.</span></span>
<span data-ttu-id="37356-319">Eles são discutidos mais detalhadamente no [ :::no-loc(Q#)::: guia da biblioteca padrão](xref:microsoft.quantum.libraries.standard.intro).</span><span class="sxs-lookup"><span data-stu-id="37356-319">These are discussed further in the [:::no-loc(Q#)::: standard library guide](xref:microsoft.quantum.libraries.standard.intro).</span></span>
...
<span data-ttu-id="37356-323">Por exemplo, o código a seguir é perfeitamente válido :::no-loc(Q#)::: , se for um pouco indireto:</span><span class="sxs-lookup"><span data-stu-id="37356-323">For instance, the following is perfectly valid :::no-loc(Q#)::: code, if a little indirect:</span></span>
...
<span data-ttu-id="37356-344">:::no-loc(Q#)::: os chamadores podem ser recursivos direta ou indiretamente.</span><span class="sxs-lookup"><span data-stu-id="37356-344">:::no-loc(Q#)::: callables are allowed to be directly or indirectly recursive.</span></span>
...
- <span data-ttu-id="37356-349">Durante a execução em um dispositivo Quantum real, o espaço de pilha pode ser limitado e, portanto, a recursão profunda pode levar a um erro de tempo de execução.</span><span class="sxs-lookup"><span data-stu-id="37356-349">When running on an actual quantum device, stack space might be limited, and so deep recursion can lead to a runtime error.</span></span>
  <span data-ttu-id="37356-350">Em particular, o :::no-loc(Q#)::: compilador e o tempo de execução não identificam e otimizam a recursão de cauda.</span><span class="sxs-lookup"><span data-stu-id="37356-350">In particular, the :::no-loc(Q#)::: compiler and runtime do not identify and optimize tail recursion.</span></span>
...
<span data-ttu-id="37356-352">Saiba mais sobre [variáveis](xref:microsoft.quantum.guide.variables) no :::no-loc(Q#)::: .</span><span class="sxs-lookup"><span data-stu-id="37356-352">Learn about [Variables](xref:microsoft.quantum.guide.variables) in :::no-loc(Q#):::.</span></span>