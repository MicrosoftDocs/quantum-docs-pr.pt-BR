---
title: 'Desenvolver com aplicativos :::no-loc(Q#)::: em um IDE'
description: 'Saiba como criar um aplicativo :::no-loc(Q#)::: que é executado no prompt de comando.'
author: bradben
ms.author: v-benbra
ms.date: 8/20/2020
ms.topic: article
...
---
# <a name="develop-with-no-locq-applications-in-an-ide"></a><span data-ttu-id="80838-103">Desenvolver com aplicativos :::no-loc(Q#)::: em um IDE</span><span class="sxs-lookup"><span data-stu-id="80838-103">Develop with :::no-loc(Q#)::: applications in an IDE</span></span>

<span data-ttu-id="80838-104">Os programas :::no-loc(Q#)::: podem ser executados por conta própria em uma linguagem de host, como C#, F# ou Python, sem a necessidade de um driver.</span><span class="sxs-lookup"><span data-stu-id="80838-104">:::no-loc(Q#)::: programs can run on their own, without a driver in a host language like C#, F#, or Python.</span></span> <span data-ttu-id="80838-105">Você pode desenvolver aplicativos :::no-loc(Q#)::: no VS Code (Visual Studio Code), no Visual Studio, nos Codespaces do Visual Studio ou em qualquer editor/IDE e executar aplicativos no console do .NET.</span><span class="sxs-lookup"><span data-stu-id="80838-105">You can develop :::no-loc(Q#)::: applications in Visual Studio Code (VS Code), Visual Studio, Visual Studio Codespaces, or with any editor/IDE and run applications from the .NET console.</span></span> 
...
<span data-ttu-id="80838-109">Embora seja possível compilar aplicativos :::no-loc(Q#)::: em qualquer IDE, é recomendável usar o VS Code (Visual Studio Code) ou o IDE do Visual Studio para desenvolver aplicativos :::no-loc(Q#)::: localmente.</span><span class="sxs-lookup"><span data-stu-id="80838-109">While you can build :::no-loc(Q#)::: applications in any IDE, we recommend using Visual Studio Code (VS Code) or Visual Studio IDE for developing your :::no-loc(Q#)::: applications locally.</span></span> <span data-ttu-id="80838-110">Para desenvolvimento na nuvem por meio do navegador da Web, recomendamos o Codespaces do Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="80838-110">For developing in the Cloud via the web browser, we recommend Visual Studio Codespaces.</span></span> <span data-ttu-id="80838-111">O desenvolvimento nesses ambientes permite aproveitar a funcionalidade sofisticada da extensão do QDK, que inclui avisos, realce de sintaxe, modelos de projeto e muito mais.</span><span class="sxs-lookup"><span data-stu-id="80838-111">Developing in these environments leverages the rich functionality of the QDK extension, which includes warnings, syntax highlighting, project templates, and more.</span></span> 
...
## <a name="develop-with-no-locq"></a><span data-ttu-id="80838-127">Desenvolver com :::no-loc(Q#):::</span><span class="sxs-lookup"><span data-stu-id="80838-127">Develop with :::no-loc(Q#):::</span></span>
...
1. <span data-ttu-id="80838-131">Clique em **Exibir** -> **Paleta de Comandos** e selecione **:::no-loc(Q#):::: Criar Novo Projeto**.</span><span class="sxs-lookup"><span data-stu-id="80838-131">Click **View** -> **Command Palette** and select **:::no-loc(Q#):::: Create New Project**.</span></span>
...
<span data-ttu-id="80838-135">Inspecione o projeto.</span><span class="sxs-lookup"><span data-stu-id="80838-135">Inspect the project.</span></span> <span data-ttu-id="80838-136">Você verá um arquivo de origem chamado `Program.qs`, um programa :::no-loc(Q#)::: que define uma operação simples para imprimir uma mensagem no console.</span><span class="sxs-lookup"><span data-stu-id="80838-136">You should see a source file named `Program.qs`, which is a :::no-loc(Q#)::: program that defines a simple operation to print a message to the console.</span></span>
...
<span data-ttu-id="80838-144">Verifique sua instalação do Visual Studio criando um aplicativo :::no-loc(Q#)::: `Hello World`.</span><span class="sxs-lookup"><span data-stu-id="80838-144">Verify your Visual Studio installation by creating a :::no-loc(Q#)::: `Hello World` application.</span></span>

<span data-ttu-id="80838-145">Para criar um novo aplicativo :::no-loc(Q#)::::</span><span class="sxs-lookup"><span data-stu-id="80838-145">To create a new :::no-loc(Q#)::: application:</span></span>
...
2. <span data-ttu-id="80838-147">Digite `:::no-loc(Q#):::` na caixa de pesquisa, selecione **Aplicativo :::no-loc(Q#):::** e clique em **Avançar**.</span><span class="sxs-lookup"><span data-stu-id="80838-147">Type `:::no-loc(Q#):::` in the search box, select **:::no-loc(Q#)::: Application** and click **Next**.</span></span>
...
<span data-ttu-id="80838-149">Inspecione o projeto.</span><span class="sxs-lookup"><span data-stu-id="80838-149">Inspect the project.</span></span> <span data-ttu-id="80838-150">Você verá um arquivo de origem chamado `Program.qs`, um programa :::no-loc(Q#)::: que define uma operação simples para imprimir uma mensagem no console.</span><span class="sxs-lookup"><span data-stu-id="80838-150">You should see a source file named `Program.qs`, which is a :::no-loc(Q#)::: program that defines a simple operation to print a message to the console.</span></span>
...
<span data-ttu-id="80838-156">Verifique sua instalação criando um aplicativo :::no-loc(Q#)::: `Hello World`.</span><span class="sxs-lookup"><span data-stu-id="80838-156">Verify your installation by creating a :::no-loc(Q#)::: `Hello World` application.</span></span>
...
1. <span data-ttu-id="80838-158">Navegue até o diretório do aplicativo:</span><span class="sxs-lookup"><span data-stu-id="80838-158">Navigate to the application directory:</span></span>
    ...
    <span data-ttu-id="80838-159">Agora, esse diretório deve conter um arquivo `Program.qs`, que é um programa :::no-loc(Q#)::: que define uma operação simples para imprimir uma mensagem no console.</span><span class="sxs-lookup"><span data-stu-id="80838-159">This directory should now contain a file `Program.qs`, which is a :::no-loc(Q#)::: program that defines a simple operation to print a message to the console.</span></span> <span data-ttu-id="80838-160">Você pode modificar esse modelo com um editor de texto e substituí-lo por seus aplicativos quânticos.</span><span class="sxs-lookup"><span data-stu-id="80838-160">You can modfiy this template with a text editor and overwrite it with your own quantum applications.</span></span> 
