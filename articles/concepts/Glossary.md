...
|{1&gt;Namespace&lt;1}|Um rótulo para uma coleção de nomes relacionados (normalmente, operações, funções e tipos). Por exemplo, o namespace [`Microsoft.Quantum.Preparation`](xref:microsoft.quantum.preparation) rotula todos os símbolos definidos na biblioteca padrão que ajudam na preparação de Estados iniciais.|
...
|Tuple|Tipos separados por vírgulas agrupados por parênteses. |
