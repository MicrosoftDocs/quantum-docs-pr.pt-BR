---
uid: Microsoft.Quantum.Math.TimesD
title: Função timesed
ms.date: 10/26/2020 12:00:00 AM
ms.topic: article
qsharp.kind: function
qsharp.namespace: Microsoft.Quantum.Math
qsharp.name: TimesD
qsharp.summary: Returns the product of two inputs.
ms.openlocfilehash: dc4f7496e74eeed269fb4e27152d1c7a8371b164
ms.sourcegitcommit: 29e0d88a30e4166fa580132124b0eb57e1f0e986
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 10/27/2020
ms.locfileid: "92696929"
---
# <a name="timesd-function"></a>Função timesed

Namespace: [Microsoft. Quantum. Math](xref:Microsoft.Quantum.Math)

Agrupa [](https://nuget.org/packages/)


Retorna o produto de duas entradas.

```qsharp
function TimesD (a : Double, b : Double) : Double
```


## <a name="input"></a>Entrada

### <a name="a--double"></a>r: [duplo](xref:microsoft.quantum.lang-ref.double)

A primeira entrada $a $ a ser multiplicada.


### <a name="b--double"></a>b: [duplo](xref:microsoft.quantum.lang-ref.double)

A segunda entrada $b $ a ser multiplicada.



## <a name="output--double"></a>Saída: [duplo](xref:microsoft.quantum.lang-ref.double)

O produto $a \times b $.